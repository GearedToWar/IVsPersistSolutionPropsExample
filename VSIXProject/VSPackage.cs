...
namespace VSIXProject
{
    ...
    public sealed class VSPackage : AsyncPackage, IVsPersistSolutionProps
    {
        /// <summary>
        /// VSPackage GUID string.
        /// </summary>
        public const string PackageGuidString = "afd7fdda-e1bb-4575-ac09-6e726fd30312";
        ...
        public int QuerySaveSolutionProps(IVsHierarchy pHierarchy, VSQUERYSAVESLNPROPS[] pqsspSave)
       {
        ...
        public int SaveSolutionProps(IVsHierarchy pHierarchy, IVsSolutionPersistence pPersistence)
        {
            ...
            if (pHierarchy == null)
            {
                return pPersistence.SavePackageSolutionProps(1, null, this, PackageGuidString);
            }
        ...
        public int ReadSolutionProps(IVsHierarchy pHierarchy, string pszProjectName, string pszProjectMk, string pszKey, int fPreLoad, IPropertyBag pPropBag)
        {
            ...
            if (pHierarchy == null)
            {
                if (PackageGuidString.Equals(pszKey, StringComparison.OrdinalIgnoreCase))
                {
                    object propertyValue;
                    pPropBag.Read("PropertyName", out propertyValue, null, (uint)VarEnum.VT_BSTR, pPropBag);

                    Assumes.Is<string>(propertyValue);
                    Assumes.Equals("PropertyValue", (string)propertyValue);
                }
            }
            return VSConstants.S_OK;
        }
